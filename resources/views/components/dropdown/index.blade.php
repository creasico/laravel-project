@props(['placement' => 'right', 'width' => '48', 'contentClasses' => 'py-1 bg-white'])
...
<div {{ $attributes->class('relative')->merge() }}
    x-data="{ open: false, placement: @js($placement), toggle: (e, placement) => {
    ...
    <button type="button" @click="toggle($event, placement); open = !open" {{ $trigger->attributes->class(['flex', 'gap-1', 'items-center', 'focus:outline-none', 'focus:text-gray-700']) }}>
        {{ $trigger }}
    </button>
